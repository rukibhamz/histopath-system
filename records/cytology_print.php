...
$id = (int)($_GET['id'] ?? 0);
$stmt = $pdo->prepare("SELECT * FROM cytology_reports WHERE id = :id");
$stmt->execute(['id' => $id]);
...
if ($can_edit) {
    $actions .= '<a class="btn" href="' . app_url('records/cytology_form.php') . '?id=' . $id . '">Edit</a>';
}
if (in_array(current_role(), ['reviewer', 'admin'], true)) {
    $actions .= '<a class="btn" href="' . app_url('records/review.php') . '?id=' . $id . '&type=cytology">Review</a>';
}
